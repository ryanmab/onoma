...
use crate::{
    models::{self},
    resolver::{
        constant::DEFAULT_SCORE,
        utils,
        weight::{self, calculate_camel_case_constant_penalty},
    },
};
...
pub fn get_fuzzy_config(query: &str) -> frizbee::Config {
    frizbee::Config {
        ...
        scoring: frizbee::Scoring {
            capitalization_bonus: weight::CASE_SENSITIVE_MATCHING_CAPITALISATION_BONUS,
...
pub fn calculate_score(
    ...
    fuzzy_matches: impl IntoIterator<Item = frizbee::Match>,
    current_file: Option<&Path>,
) -> Option<i64> {
    let fuzzy_match_bonus = fuzzy_matches
        .into_iter()
        .map(weight::calculate_fuzzy_match_bonus)
        .sum();

    if fuzzy_match_bonus == 0 && !query.is_empty() {
        // The symbol was pre-filtered out of the queries fuzzy match, meaning we
        // can stop here.
        return None;
    }
    ...
    let clear_intent_bonus: i64 = weight::calculate_clear_intent_bonus(query, symbol);
    ...
    let autogenerated_code_penalty = if utils::is_autogenerated_code(&symbol.path) {
        // Penalty for symbols which were (likely) autogenerated by developer tools. For example,
        // GraphQL Codegen, Protobuf, etc.
    ...
    Some(
        DEFAULT_SCORE
            ...
            .saturating_add(calculate_camel_case_constant_penalty(symbol)),
    )
}
...
mod tests {
    ...
    pub fn test_scoring_struct_in_entrypoint_file() {
        ...
        let score = super::calculate_score("", &symbol, None, None);

        let mut target_score = DEFAULT_SCORE;

        target_score += 70; // Increase the score by 7%, because it is a struct
        target_score -= 5; // Reduce the default score by 0.5% because the Symbol is in a module file

        assert_eq!(Some(target_score), score);
    }
    ...
    pub fn test_scoring_variable_in_far_away_file() {
        ...
        target_score += 30; // Increase the score by 3%, because it is a variable
    ...
    pub fn test_scoring_variable_in_same_file() {
        ...
        target_score += 30; // Increase the score by 3%, because it is a variable
        target_score -= 20; // Reduce the default score by 2%
    ...
    pub fn test_scoring_module_symbol() {
        ...
        target_score -= 30; // Decrease the score by 1%, because it is a variable
    ...
    pub fn test_scoring_class_in_test_file() {
        ...
        target_score += 70; // Increase the score by 7%, because it is a Class
        target_score -= 20; // Decrease the score by 2.0%, because its in a test file
    ...
    pub fn test_scoring_type_in_autogenerated_folder() {
        ...
        target_score += 70; // Increase the score by 7%, because it is a Class
        target_score -= 5; // Decrease the score by 0.5%, because it's an entrypoint file
        target_score -= 100; // Decrease the score by 14%, because its in an autogenerated folder
    ...
    pub fn test_scoring_fuzzy_matched_symbol() {
        ...
        let score = super::calculate_score(
            query,
            &symbol,
            frizbee::Matcher::new(query, &config).match_one(name.as_str(), 0),
            None,
        );

        let mut target_score = DEFAULT_SCORE;

        target_score += 26; // Increase the score by 5.2% for the fuzzy matches (with matching case)
    ...
    pub fn test_scoring_fuzzy_matched_symbol_smartcase_insensitive() {
        ...
        target_score += 24; // Increase the score by 4.8% for the fuzzy matches (as the case does not match)
    ...
    fn test_constant_screaming_case_intent(#[case] kind: SymbolKind) {
        ...
        let score = weight::calculate_clear_intent_bonus(query, &sym);
